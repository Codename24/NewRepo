...
namespace TaxCalculator.API.Controllers
{
    ...
    public class TaxController : ControllerBase
    {
            ...
            [FromRoute] int annualSalary)
        {
            ...
            var taxResult = await _mediator.Send(new CalculateTaxCommand { AnnualSalary = annualSalary });

            if (taxResult != null)
            {
                await _mediator.Send(new SaveTaxResultCommand { TaxResult = taxResult });
                _logger.LogInformation("Tax result saved for Annual Salary: {AnnualSalary}", annualSalary);
            }
